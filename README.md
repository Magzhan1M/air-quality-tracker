# air-quality-tracker
A no-code web app for tracking real-time air quality with map view and health tips.
# Air Quality Tracker

A mobile app to monitor real-time air quality and give users health advice based on pollution levels.

## Features
- Live air quality map (PM2.5, PM10, etc.)
- Location-based pollution levels
- Health recommendations
- User-friendly interface

## ⚙️ Built With
- No-code platform: [Thunkable](https://thunkable.com) 
- Open data API for air quality: [Openweathermap] (https://openweathermap.org), [Weatherbit] (https://www.weatherbit.io)
- Visual design with built-in Thunkable UI components  

## 📷 Screenshots
![IMG_1592](https://github.com/user-attachments/assets/44ddf238-e6ab-4cc1-b9a3-0d4fdbe09352) ![IMG_1587](https://github.com/user-attachments/assets/ad72ec2f-b638-43f1-aef8-87b6de5136ed)


## 🔗 Try the App
[Link to the app](https://thunkable.site/w/lS94ErY_8Kfw60wn72Kwb)  
